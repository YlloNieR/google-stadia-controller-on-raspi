...
for event in controller.read_loop():
    ...
    # Check for buttons for mouse actions
        ...
        # A Button - like left click on mouse
    ...
    #     print("B button pressed -> ALT + LEFT")

    #     device.emit(uinput.KEY_LEFTALT, 1)   # ALT drücken
    #     device.emit(uinput.KEY_LEFT, 1)      # LEFT drücken
    #     device.syn()                         # Synchronisation
    #     time.sleep(0.1)                      # Längere Verzögerung

    #     device.emit(uinput.KEY_LEFTALT, 0)   # ALT loslassen
    #     device.emit(uinput.KEY_LEFT, 0)      # LEFT loslassen
    #     device.syn()                         # Synchronisation
    #     time.sleep(0.1)                      # Längere Verzögerung


    # Y Button - like right click on mouse
    if event.code == ecodes.BTN_WEST and event.value == 1:  # Y pressed
        print("Y button pressed -> Right click")
        device.emit(uinput.BTN_RIGHT, 1)
        device.syn()  # Synchronize the input
    elif event.code == ecodes.BTN_WEST and event.value == 0:  # Y released
        device.emit(uinput.BTN_RIGHT, 0)
        device.syn()  # Synchronize the input

    # Burger menue - Open Chromium when the START button (BTN_START) is pressed
    if event.code == ecodes.BTN_START and event.value == 1:
        print("Burger menue button pressed -> Opening Chromium")
        subprocess.Popen(["chromium-browser"])  # Launch Chromium

    # R1 - Simulate ALT + TAB + TAB when BTN_TR is pressed
    if event.code == ecodes.BTN_TR and event.value == 1:
        print("R1 button pressed -> Switching window")
    ...
    # L1 - Simulate ALT + SHIFT + TAB for backward (left) window switching
    if event.code == ecodes.BTN_TL and event.value == 1:  # Example button for backward switching
        print("L1 button pressed -> Switching to the previous window")
        device.emit(uinput.KEY_LEFTALT, 1)   # Press ALT
    ...
    # Assitant - Stop the script when the Assitant button (BTN_TRIGGER_HAPPY1) is pressed
    if event.code == ecodes.BTN_TRIGGER_HAPPY1 and event.value == 1:
        print("Assitant button pressed -> Exiting")
        break

    # Keyboard - ESC - Exit with the ESC key for a clean termination
